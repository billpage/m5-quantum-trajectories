...
The **osmotic velocity** u = ν ∂ₓ ln ρ is obtained from the √ρ-weighted selection mechanism without differentiating the density — this is the Method 5 innovation over standard DHV methods that require explicit computation of ∇ ln ρ. The drift can also be decomposed purely through selection operations: max-selection gives +u, min-selection gives −u, and their average and difference recover the current velocity v and osmotic velocity u independently (see main document §2.1).

The **osmotic divergence** u' = ∂ₓu is obtained from the ln ρ mean-weight readout — the Method 5 (updated) innovation that accesses Holland's backward sector.
...
### 17.3 The Sparse Ontology Question
...
### 17.4 Connection to the Bipolar Ansatz
...
This distinction is crucial for Method 5 (updated): because σ± = S ± R are algebraically determined by (S, ρ), a single ensemble carrying (X, S) suffices. The bipolar ansatz requires two independent ensembles (or equivalently, a doubled state space), while Holland's decomposition requires only a single ensemble with one additional readout (the log-density mean weight) to access the backward sector.

The Method 7 implementation (§18) explored the bipolar-like approach of maintaining two separate ensembles and found that the anti-dissipative ("−") ensemble develops caustics and requires periodic resampling. The Method 5 (updated) approach avoids this entirely by staying within a single forward-evolving ensemble.

---

## 18. Lessons from the Two-Species Method 7

### 18.1 The Dissipative/Anti-Dissipative Asymmetry

The Method 7 prototype (developed in parallel with the updated algorithm) implemented Holland's two-congruence structure literally: "+" particles propagated stochastically with √ρ selection (drift v₊ = v + u), while "−" particles propagated deterministically with drift v₋ = v − u.
...
### 18.2 The Inescapable Conclusion and the Resolution
...
## 19. The Full Schrödinger Bridge Interpretation

### 19.1 Holland's bi-HJ as an FBSDE Optimality System
...
### 19.2 The Three Readouts as Bridge Components

The readout table in §8.3 of the main document now has a clean bridge interpretation:
...
### 19.3 Anti-Diffusion Without Backward Simulation
...
The Method 5 (updated) log-density readout computes (2/ν)u' = ∂²ₓ(ln ρ) directly. Combined with Q = −(ℏ²/2m)(g''/g), which gives (u² + νu')/ν², the backward curvature is fully determined.

The crucial insight: **the anti-diffusive equation's content is an instantaneous property of the density, not a time-evolved quantity.** The backward heat equation is ill-posed as a dynamical evolution, but its Laplacian (the only information needed for the coupling terms) is a well-conditioned local functional of ρ. The Method 5 (updated) algorithm extracts this functional through the GH quadrature of ln ρ, converting an ill-posed differential operation into a well-conditioned integral one.

This is the same mathematical trick that underlies the v2 WEIGH readout: the Gaussian mean-value property converts differentiation into averaging. What the updated algorithm adds is the recognition that applying this trick to **ln ρ** instead of **√ρ** gives access to the backward sector with no additional ill-conditioning.

### 19.4 The Sign of the Fisher Information
...
## 20. Self-Consistency and Quantum Equilibrium

### 20.1 Method 5 Is Not Pilot-Wave Theory
...
### 20.2 The Self-Consistency Loop
...
This is guaranteed by the proofs in the main document (Theorems 1 and 2): the √ρ selection induces exactly the osmotic drift u = ν∂ₓ ln ρ, and the mean weight gives exactly the quantum potential Q = −(ℏ²/2m)(∂²ₓ√ρ/√ρ), both to O(dt) per step. With GH quadrature nodes (the preferred approach), the Q estimate has no stochastic variance — only an O(σ_gh²) deterministic bias that does not accumulate. With random antithetic candidates (the fallback approach), the error accumulates as a random walk σ_S = |u|√(T/K), which is controlled by taking K sufficiently large. In either case, with appropriate K the self-consistency is maintained to adequate precision over any finite time interval (see main document §9.1–9.3 for the full variance comparison).

### 20.3 The (X, S) Representation
...
## 21. Wasserstein Dynamics and the Newton Equation

### 21.1 The Density as a Particle in Wasserstein Space
...
### 21.4 The Elastic Membrane Analogy
...
## 22. Related Literature: Derivative Estimation from Gaussian Perturbations
...
### 22.1 The Mean Value Property and Harmonic Function Theory
...
### 22.2 Walk on Spheres and Monte Carlo PDE Solvers
...
### 22.3 Zeroth-Order Optimisation
...
### 22.4 Stein's Method and Score Matching
...
The Method 5 log-density readout (Theorem 3 (main document §5.3)) is closely related: it extracts ∂²ₓ(ln ρ) from the GH mean of ln ρ, which is equivalent to score-matching applied to the particle density. The connection is:

    ∂²ₓ(ln ρ) = ∂ₓ(score) = Stein operator applied to ρ

In the diffusion model literature (Song & Ermon 2019, Generative modeling by estimating gradients of the data distribution), the score ∇ ln p is estimated from noisy observations by denoising, and the Laplacian of the score controls the diffusion dynamics. Method 5 (updated)'s log-density readout provides the Laplacian of the score (= u' in our notation) directly, without a denoising step — the GH quadrature serves as an exact score-Laplacian estimator for smooth densities.

### 22.5 Diffusion Monte Carlo
...
### 22.6 What Is Novel in the Method 5 Synthesis
...
3. **Triple readout with the log-density channel.** The addition of ln ρ as a third weight function, providing the osmotic divergence u' independently of Q, extends the dual-readout framework to access the full Holland bi-HJ structure. The combination (√ρ mean → Q, ln ρ mean → u', algebra → Q̃) is new.
...
### 23.1 The Ground State Challenge
...
The osmotic divergence readout provides a new diagnostic and potentially a new controller signal: if u'_measured ≠ −ω (the exact value), the density estimate is unreliable. The u' error could supplement or replace the energy error as a feedback signal for the adaptive bandwidth controller.
...
Method 5 (updated) provides a complete, derivative-free, particle-level implementation of Holland's bi-Hamilton–Jacobi system for quantum dynamics. The algorithm uses a single forward-evolving ensemble carrying (X, S) per particle, with three readouts from a shared Gauss–Hermite candidate cloud:

1. **√ρ selection** → forward osmotic drift u
2. **√ρ mean weight** → forward quantum potential Q
3. **ln ρ mean weight** → osmotic divergence u' (added in updated algorithm)
4. **Algebraic reconstruction** → backward quantum potential Q̃ = Q + ℏu' (added in updated algorithm)
5. **Mirror particles** → boundary-corrected KDE (added in updated algorithm)
...
- `NelsonMechanics_SchrodingerBridge_Algorithm.md` — Main document: core algorithm, theorems, GH quadrature, backward channel, numerical validation
- `NelsonMechanics_SchrodingerBridge_Supplement.md` — This document: bridge interpretation, Wasserstein dynamics, Hackebill–Poirier classification, open questions
